  ...
  struct Polyline_visitor{
    ...
    void add_node(AL_vertex_descriptor node_id)
    {
      ...
      if (const vertex_descriptor* vd_ptr = boost::get<vertex_descriptor>(&v) )
      {
        ...
        current_edge.push_back(get(m_vpmap, *vd_ptr));
      }
      else
      {
        edge_descriptor ed = boost::get<edge_descriptor>(v);
        current_edge.push_back(ed);
//        Segment_3 s(
...
//      *out++=current_poly;
      ...
      CGAL_assertion(!current_edge.empty());
      *out++=current_edge;
    }
  };
...
/// Other private functions
  ...
  void update_al_graph_connectivity(edge_descriptor ed,AL_vertex_descriptor vd,AL_edge_map& al_edge_map,AL_graph& al_graph) const
  {
